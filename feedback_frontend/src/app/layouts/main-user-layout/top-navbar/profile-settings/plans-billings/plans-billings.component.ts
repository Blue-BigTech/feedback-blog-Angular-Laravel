...
export class PlansBillingsComponent implements OnInit {
  countries = [
  ...
  paymentInformation: boolean = false;
  paymentHistory: boolean = false;

  billingInformationForm: boolean = false;
  paymentInformationForm: boolean = false;

  paymentMethod: string = 'credit-card';

  default: boolean = true;

  userCountry: string = 'Choose Country';

  cardExpirationMonth: string = 'MM';
  cardExpirationYear: string = 'YY';
  cvvPreview: boolean = false;
  ...
  editBillingInfo(): void {
    this.billingInformationForm = !this.billingInformationForm;
  }
  editPaymentInfo(): void {
    this.paymentInformationForm = !this.paymentInformationForm;
  }
  ...
  chooseExpirationMonth(MM: string): void {
    this.cardExpirationMonth = MM;
  }

  chooseExpirationYear(YY: string): void {
    this.cardExpirationYear = YY;
  }
  ...
  inputCardExpirationMonth(event: any): void {
    event.target.value = event.target.value.replace(/[^\d]/g, '');
    ...
    event.target.value = event.target.value.replace(/1[3456789]/g, '1');
  ...
  inputCardExpirationYear(event: any): void {
    event.target.value = event.target.value.replace(/[^\d]/g, '');

    event.target.value = event.target.value.replace(/^[01456789]/g, '');

    event.target.value = event.target.value.replace(/^2[01]/g, '2');
