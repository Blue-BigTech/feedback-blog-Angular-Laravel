...
export class WorkspaceLocalstorageComponent implements OnInit {
  ...
  config: IConfig = null || defaultConfig

  ngOnInit() {
    if (localStorage.getItem(this.localStorageName)) {
      this.config = JSON.parse(localStorage.getItem(this.localStorageName)+'')
      console.log('cccccccccccccccccccc',this.config.columns);
    } else {
      this.resetConfig()
    }
  }
  ...
  onDragEnd(columnindex: number, e: { gutterNum: number; sizes: Array<number> } | any) {
    // Column dragged
    if (columnindex === -1) {
